...
def main():
    ...
    # 用来保存coco_info的文件
    results_file = "results{}.txt".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))       # format在{}中填充此刻实时时间信息，形成一个txt文件名
    ...
    data_transform = {                                                         # 预处理
        "train": transforms.Compose([transforms.ToTensor(),
                                     transforms.RandomHorizontalFlip(0.5)]),   # 水平随即翻转图像，随之标签也需要翻转
        "val": transforms.Compose([transforms.ToTensor()])
    }

    VOC_root = "./"  # VOCdevkit   "./"表示当前路径
